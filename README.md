...
- **Public nav** is the earlier floating pill nav (How It Works, Pricing, Site Intelligence, Log in). The pricing page shows a single Business plan (the Residents and Government plans were removed). The "simulated data" banner is hidden for simulated readings (it still appears for an imported snapshot, which is not live), and the hardware-connection link moved into the context row. The "simulated / synthetic / conceptual" wording was also removed from the footer, landing labels, Operations and Costs captions and the landscape caption, and the SMS preview alert was replaced by an audible alarm. The pricing cards were redesigned (gradient Business card, check icons, pill buttons). Kept on purpose: the not-certified compliance disclaimer (required), the "illustrative dataset" note on Site intelligence (those candidate sites are fictional), the FAQ answers about savings and compliance, the "Business demo" badge, and the limits documented here. Note that the app's readings are still generated by a simulator, so this README is where that is stated.
- **Threshold alarm** (`web/assets/alarm.js`): switch on "Sound an alarm on exceedance" on the Costs page and, whenever the level goes over the applicable day/night limit, a two-tone siren (Web Audio, no files or services) repeats and a banner appears on every page until you press Silence or the level returns under the limit; it stops on its own after 60 seconds and on sign-out. It never plays until the user switches it on, works with a visual banner alone, and does not pulse under reduced motion. It is a local alert in the open browser tab, not a remote notification. Two settings control when it sounds: an **alert level** (at the limit, or 1, 2, 3 or 5 dB under it, so operators get warned before a breach) and a **sustained-for** time (immediately, 10 s, 30 s, 1 min or 5 min, so a brief spike does not trigger it; default 30 s). A live line under the settings shows the resulting level. The limits themselves are edited through "Edit profile". The legal-limit exceedance log is separate and still records the first reading over the configured limit.
