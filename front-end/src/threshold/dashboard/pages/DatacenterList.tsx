  ...
  return (
    <div className="min-h-screen bg-[color:var(--bg)] text-[color:var(--text)]">
      ...
      <main className="mx-auto max-w-6xl px-6 py-10">
        <div className="mb-8 flex flex-col gap-4 rounded-[24px] glass p-6 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <div className="text-xs font-medium text-[color:var(--text-dim)]">Operator account</div>
        ...
        <h1 className="mb-1 text-3xl font-light tracking-tight text-[color:var(--text-h)]">Monitored facilities</h1>
          ...
          {DATACENTERS.map((dc) => {
            ...
            return (
              <Link
                key={dc.id}
                to={`/dashboard/${dc.id}`}
                className="glass group rounded-[24px] p-6 transition-transform duration-200 ease-out hover:-translate-y-0.5"
              >
